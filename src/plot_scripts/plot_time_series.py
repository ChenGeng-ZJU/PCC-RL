...
def plot_time(log_file, trace_file, save_dir):
    noise = 0
    for log_idx, log_file in enumerate(log_file):
        ...
        if PLOT:
            ...
            if trace_file:
                ...
                elif trace_file.endswith('.log'):
                    trace = Trace.load_from_pantheon_file(trace_file, delay=5, loss=0, queue=10)
                else:
                    raise RuntimeError
            ...
            if noise != 0:
                axes[0].set_title('noise ~ max(0, N(0.004, {})'.format(noise))
